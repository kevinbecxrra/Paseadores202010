...
public class RecorridoPuntoLogicTest {

    private PodamFactory factory = new PodamFactoryImpl();

    @Inject
    private RecorridoPuntoLogic puntoRecorridoLogic;
    ...
    private void insertData() {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            data.add(entity);
            if (i == 0) {
                puntosData.get(i).setRecorrido(entity);
            }
        }
    }

    /**
     * Prueba para asociar un Puntos existente a un Recorrido.
     */
    @Test
    public void addRecorridoTest() {
        RecorridoEntity entity = data.get(0);
        PuntoEntity puntoEntity = puntosData.get(1);
        RecorridoEntity response = puntoRecorridoLogic.addRecorrido(entity.getId(), puntoEntity.getId());
    ...
    public void getRecorridoTest() {
        PuntoEntity entity = puntosData.get(0);
        RecorridoEntity resultEntity = puntoRecorridoLogic.getRecorrido(entity.getId());
    ...
    public void getPuntosTest() {
        PuntoEntity entidad = puntosData.get(0);
        RecorridoEntity resultado = puntoRecorridoLogic.getRecorrido(entidad.getId());
        List list = puntoRecorridoLogic.getPuntos(resultado.getId());
    ...
    public void getPuntoTest() {
        PuntoEntity ent = puntosData.get(0);
        RecorridoEntity res = puntoRecorridoLogic.getRecorrido(ent.getId());
        try{
        Assert.assertNotNull(puntoRecorridoLogic.getPunto(res.getId(), ent.getId()));
        }
        catch(Exception e){}
    ...
    /**
     * Prueba para remplazar las instancias de Puntos asociadas a una instancia
     * de Recorrido.
     */
    @Test
    public void replaceRecorridoTest() {
        RecorridoEntity entity = data.get(0);
        puntoRecorridoLogic.replaceRecorrido(puntosData.get(1).getId(), entity.getId());
        entity = puntoRecorridoLogic.getRecorrido(puntosData.get(1).getId());
        Assert.assertTrue(entity.getPuntos().contains(puntosData.get(1)));
    }

    /**
     * Prueba para desasociar un Punto existente de un Recorrido existente.
     *
     */
    @Test
    public void removePuntoTest(){
        puntoRecorridoLogic.removePunto(puntosData.get(0).getId());
        Assert.assertNull(puntoRecorridoLogic.getRecorrido(puntosData.get(0).getId()));
    }
